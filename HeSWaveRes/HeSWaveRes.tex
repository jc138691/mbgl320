...
FC (Frozen-core, $N_c=1$, $N_t=30$), JM ($N_c=7$, $N_t=30$) and CCC ($N_c=3$, $N_t=30$) results  were shifted to the right by 0.17747eV, 0.0018eV and 0.0018eV (Table~\ref{Tab_ENGS}), respectively.
}
\label{Fig_1}
\end{figure*}
...
the first seven significant figures \cite{G94} were obtained by using  $\{R^{c}_p(r)\}_{p=0}^{N_c-1}$ as the basis for both electrons with $N_c=50$ and $\lambda_c=4$, denoted by $(50,50)$ in Table~\ref{Tab_ENGS}.

Fig.~\ref{Fig_1} shows that if the FC results are shifted (as they are on all the figures) to the right by the FC ground energy error (0.17747eV, see Table~\ref{Tab_ENGS}),
