 ...
 */
public abstract class JmMethodBaseE2 extends JmMethodBaseE1 {
  ...
  protected JmCh[] loadChArr(double sysEng) {
    JmLgrrModel jmModel = jmOpt.getJmModel();
    ...
    for (int i = 0; i < tEngs.size(); i++) {     log.dbg("i = ", i);
      ...
      res[i] = new JmCh(sysEng, tEngs.get(i), jmModel, -trgtE2.getScreenZ());      log.dbg("res[i]=", res[i]);
    }
    return res;
  }
  private boolean isValidD(Vec overD, int nt) {
    ...
    if (overD.size() <= nt)   // something is wrong!
      return false;
    double maxErr = jmOpt.getJmTest().getMaxIntgrlErr();
  ...
  public JmRes calc_OLD(Vec engs) {          //JmMethodJmBasisE3.log.setDbg();
    JmRes res = new JmRes();
    int cN = getChNum();
    int eN = engs.size();
    ...
    for (int i = 0; i < eN; i++) {       log.info("i = ", i);
      ...
      CmplxMtrx WCJC;
      CmplxMtrx WSJS;
      if (jmOpt.getUseClosed()) {
        W = calcW(sysE, X.getArray());       log.dbg("W=\n", new MtrxDbgView(W));
        WCJC = calcWCJC(W, chArr);      log.dbg("WCJC=\n", new CmplxMtrxDbgView(WCJC));
        WSJS = calcWSJS(W, chArr);      log.dbg("WSJS=\n", new CmplxMtrxDbgView(WSJS));
      }
      else {
        W = calcOpenW(chArr, sysE, X.getArray());    log.dbg("W=\n", new MtrxDbgView(W));
        WCJC = calcOpenWCJC(W, chArr);          log.dbg("WCJC=\n", new CmplxMtrxDbgView(WCJC));
        WSJS = calcOpenWSJS(W, chArr);          log.dbg("WSJS=\n", new CmplxMtrxDbgView(WSJS));
      }

      CmplxMtrx R = calcR(WCJC, WSJS, chArr);      log.dbg("R=\n", new CmplxMtrxDbgView(R));
      CmplxMtrx iRm = calcIR(R, chArr, -1);      log.dbg("(1-iR)=\n", new CmplxMtrxDbgView(iRm)); // (1-iR) open channels only
      CmplxMtrx iRp = calcIR(R, chArr, +1);      log.dbg("(1+iR)=\n", new CmplxMtrxDbgView(iRp));// (1+iR) open channels only
      ...
      CmplxMtrx mS = iRp.times(iRmInv);             log.dbg("S matrix=\n", new CmplxMtrxDbgView(mS));
      calcCrossSecs(i, res, mS);
      calcSdcs(i, res);
      //      calcSdcsCurveFit(i, res);  // step-function SDCS
    }
  ...
  protected void calcSdcs(int i, JmRes res) {
    loadSdcsW(chArr);
  ...
  protected void calcSdcsFromW(int i, JmRes res) {
    int tN = getChNum();
    Mtrx mCs = res.getCrossSecs();
...
//    return calc(scttEngs);
  ...
  //  private void calcSdcsCurveFit(int i, JmRes res) {
  //    double midCs = calcMidSdcs(i, res);
  //    makeStepLinear(i, res, midCs);
  //    makeStepCube(i, res, midCs);
  //  }
  //  private Func makeQuadFit(int i, JmRes res) {
  ...
  //    };
  //  }
  //  private void makeStepCube(int i, JmRes res, double midCs) {
  //    JmCh ch0 = chArr[0];
  ...
  ////    return makeStepFunc(a, b, midE);
  //  }
  //  private void makeStepLinear(int i, JmRes res, double midCs) {
  //    JmCh ch0 = chArr[0];
  //    final double midE = 0.5 * ch0.getSysEng();
  //    final double b = 4. * midCs;
  //    double tics = res.getTics().get(i, IDX_IONIZ);
  ...
  ////    };
  //  }
  //  private double calcMidSdcs(int i, JmRes res) {
  //    Mtrx mSdcs = res.getSdcs();
  //    JmCh ch0 = chArr[0];
  //    double midE = 0.5 * ch0.getSysEng();
  //    int tN = getChNum();
