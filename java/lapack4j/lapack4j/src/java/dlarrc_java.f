...
public static void DLARRC( JOBT, N, VL, VU, D, E, PIVMIN,EIGCNT, LCNT, RCNT, INFO ) { //SUBROUTINE DLARRC( JOBT, N, VL, VU, D, E, PIVMIN,EIGCNT, LCNT, RCNT, INFO )
//*
...
int EIGCNT, INFO, LCNT, N, RCNT; //INTEGER            EIGCNT, INFO, LCNT, N, RCNT
...
LPIVOT = D( 1 ) - VL; //LPIVOT = D( 1 ) - VL
RPIVOT = D( 1 ) - VU; //RPIVOT = D( 1 ) - VU
...
LPIVOT = D( N ) + SL; //LPIVOT = D( N ) + SL
RPIVOT = D( N ) + SU; //RPIVOT = D( N ) + SU
if ( LPIVOT <= ZERO ) { //IF( LPIVOT.LE.ZERO ) THEN
LCNT = LCNT + 1; //LCNT = LCNT + 1
} // ENDIF
if ( RPIVOT <= ZERO ) { //IF( RPIVOT.LE.ZERO ) THEN
