...
public static void DLARRR( N, D, E, INFO ) { //SUBROUTINE DLARRR( N, D, E, INFO )
//*
...
{  ZERO = 0.0D0,RELCOND = 0.999D0 ; } //PARAMETER          ( ZERO = 0.0D0,RELCOND = 0.999D0 )
//*     ..
//*     .. Local Scalars ..
int I; //INTEGER            I
...
TMP = SQRT(ABS(D(1))); //TMP = SQRT(ABS(D(1)))
if  (TMP < RMIN) YESREL = .FALSE.; //IF (TMP.LT.RMIN) YESREL = .FALSE.
if (.NOT.YESREL) GOTO 11; //IF(.NOT.YESREL) GOTO 11
for ( 10 I = 2; N) { //DO 10 I = 2, N
TMP2 = SQRT(ABS(D(I))); //TMP2 = SQRT(ABS(D(I)))
if  (TMP2 < RMIN) YESREL = .FALSE.; //IF (TMP2.LT.RMIN) YESREL = .FALSE.
if (.NOT.YESREL) GOTO 11; //IF(.NOT.YESREL) GOTO 11
OFFDIG2 = ABS(E(I-1))/(TMP*TMP2); //OFFDIG2 = ABS(E(I-1))/(TMP*TMP2)
if (OFFDIG+OFFDIG2 >= RELCOND) YESREL = .FALSE.; //IF(OFFDIG+OFFDIG2.GE.RELCOND) YESREL = .FALSE.
if (.NOT.YESREL) GOTO 11; //IF(.NOT.YESREL) GOTO 11
TMP = TMP2; //TMP = TMP2
