...
public class DLARRA { //SUBROUTINE DLARRA( N, D, E, E2, SPLTOL, TNRM,NSPLIT, ISPLIT, INFO )
  public static void DLARRA(
    ...
    , double SPLTOL  //SPLTOL  (input) DOUBLE PRECISION
    , double TNRM  //TNRM    (input) DOUBLE PRECISION
    , IntRef pNSPLIT  //NSPLIT  (output) INTEGER
    ...
    //*
    //*     .. Scalar Arguments ..
    int INFO = 0, NSPLIT = 0; //INTEGER            INFO, N, NSPLIT
    ...
    if (SPLTOL < ZERO) { //IF(SPLTOL.LT.ZERO) THEN
      ...
      for (I = 1; I <= N-1; I++) { //DO 9 I = 1, N-1
        ...
        if ( EABS  <=  TMP1) { //IF( EABS .LE. TMP1) THEN
          ...
          ISPLIT[ NSPLIT -1] = I; //ISPLIT( NSPLIT ) = I
    ...
    ISPLIT[ NSPLIT -1] = N; //ISPLIT( NSPLIT ) = N
    pINFO.set(INFO);
  } // RETURN
  //*
  //*     End of DLARRA
