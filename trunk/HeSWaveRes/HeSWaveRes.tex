...
the following three sets of functions were used: {\em target basis}, {\em JM functions}, and {\em Laguerre basis}.
{\em Target basis} is the a set of $N_t$ orthonormal radial functions $\{P_n(r)\}_{n=1}^{N_t}$,
...
While the $He$ excitations resemble $ns$-orbitals of hydrogen and therefore are long-range.
This problem is solved here by using a mix of the short range $\{R^c_n(r)\}_{n=1}^{N_c}$ with $\lambda_c=4$  
and long-range $\{R^t_n(r)\}_{n=N_c+1}^{N_t}$ $\lambda_t=1$  basis sets
when constructing the target basis.

TODO: explain KFB basis construction
...
$\lambda_c=4$, $\lambda_t=1$, $N_t=30$, $N=10$, $\ln(c)=-5 - 2\ln(Z_{\mbox{\tiny{He}}} )$, $Z_{\mbox{\tiny{He}}}=2$, $r_{\max}=500$,
...
CHECK PhysRev email on how to upload data to them.
