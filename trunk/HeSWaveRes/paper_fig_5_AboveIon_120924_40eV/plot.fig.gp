...
set style line 14 lt 1 lc 2    lw 3  pt 7 ps 1 

set term postscript eps enhanced lw 1 size 18cm,14cm solid color 18 "fixed"; set out 'fig5.ps'
...
set xrange [E_min:60];  set xtics 5;    set mxtics 5;   set format x "%3.0f"; 
...
p \
  ...
  JM_3 u ($1*xJM):($2*yJM) t LABEL_3 w l ls 13, \
...
p \
  ...
  TICS_3 u ($1*xJM):($2*yJM*100) t '' w l ls 13, \
...
set yrange [1.2:1.65];           set ytics 0.1;  set mytics 5;   set format y "%3.1f"; 
...
p \
  ...
  JM_3 u ($1*xJM):($5*yJM*1000) t '' w l ls 13, \
...
p \
  ...
  JM_4 u ($1*xJM):($6*yJM*1000) t LABEL_4 w l ls 14,\
