...
/**
 * Copyright dmitry.konovalov@jcu.edu.au Date: 9/09/2008, Time: 12:09:34
 */
public class StrVec extends TVec<String> {
  public static Log log = Log.getLog(StrVec.class);

  public StrVec(String[] from) {
    super(from);
  }
   public StrVec(StrVec from) {
    super(from.getArr());
  }
  ...
  {
    log.dbg("arr=", new StrVec(arr));
    log.dbg("arr2=", new StrVec(arr2));
    int L = arr.length;
    int L2 = arr2.length;
    ...
    log.dbg("res=", new StrVec(res));
    return res;
  }

  public static String[] get(BitSet set, String[] from)
  {
    log.dbg("select(set=", set);
    log.dbg("from=", new StrVec(from));
  ...
  {
    log.dbg("order(idxOrder=", new IntVec(idxOrder));
    log.dbg("from=", new StrVec(from));
