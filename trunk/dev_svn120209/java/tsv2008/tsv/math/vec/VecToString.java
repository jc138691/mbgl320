...
public static String toString(Mtrx arr, int row, String delim) {
  return toString(arr, row, arr.getNumRows(), delim);
}
...
public static String toString(Mtrx a, int row, int size, String delim) {
  int L = Math.min(a.getNumCols(), size);
  StringBuffer buff = new StringBuffer();
  for (int c = 0; c < L; c++) {
    double v = a.get(row, c);
    buff.append(Double.toString(v));
    if (c != L - 1)
      buff.append(delim);
  }
...
public static String toString(Mtrx a, int row, int size, String delim, int digs) {
  NumberFormat format = NumberFormat.getNumberInstance();
  format.setMaximumFractionDigits(digs);
  int L = Math.min(a.getNumCols(), size);
  StringBuffer buff = new StringBuffer();
