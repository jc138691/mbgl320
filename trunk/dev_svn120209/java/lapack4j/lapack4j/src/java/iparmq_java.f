...
public static int IPARMQ( ISPEC, NAME, OPTS, N, ILO, IHI, LWORK ) { //INTEGER FUNCTION IPARMQ( ISPEC, NAME, OPTS, N, ILO, IHI, LWORK )
//*
...
//*     
//*     .. Scalar Arguments ..
int IHI, ILO, ISPEC, LWORK, N; //INTEGER            IHI, ILO, ISPEC, LWORK, N
...
REAL               TWO; //REAL               TWO
{  TWO = 2.0 ; } //PARAMETER          ( TWO = 2.0 )
//*     ..
//*     .. Local Scalars ..
int NH, NS; //INTEGER            NH, NS
//*     ..
//*     .. Intrinsic Functions ..
//INTRINSIC          LOG, MAX, MOD, NINT, REAL
//*     ..
//*     .. Executable Statements ..
if ( ( ISPEC == ISHFTS )  ||  ( ISPEC == INWIN )  || ( ISPEC == IACC22 ) ) { //IF( ( ISPEC.EQ.ISHFTS ) .OR. ( ISPEC.EQ.INWIN ) .OR.( ISPEC.EQ.IACC22 ) ) THEN
//*
//*        ==== Set the number simultaneous shifts ====
//*
NH = IHI - ILO + 1; //NH = IHI - ILO + 1
NS = 2; //NS = 2
if ( NH >= 30 )NS = 4; //IF( NH.GE.30 )NS = 4
if ( NH >= 60 )NS = 10; //IF( NH.GE.60 )NS = 10
if ( NH >= 150 )NS = MAX( 10, NH / NINT( LOG( REAL( NH ) ) / LOG( TWO ) ) ); //IF( NH.GE.150 )NS = MAX( 10, NH / NINT( LOG( REAL( NH ) ) / LOG( TWO ) ) )
if ( NH >= 590 )NS = 64; //IF( NH.GE.590 )NS = 64
if ( NH >= 3000 )NS = 128; //IF( NH.GE.3000 )NS = 128
if ( NH >= 6000 )NS = 256; //IF( NH.GE.6000 )NS = 256
