...
//import static lapack4j.src.java.DLARRA.DLARRA;
//import static lapack4j.src.java.DLARRD.DLARRD;
///**
...
//public static void DLARRE(
//  char RANGE   //RANGE   (input) CHARACTER*1
//  , final int N  // N       (input) INTEGER
//  , double[] VL  //VL      (input/output) DOUBLE PRECISION
//  , double[] VU  //VU      (input/output) DOUBLE PRECISION
//  , final int IL  //IL      (input) INTEGER
//  , final int IU  //IU      (input) INTEGER
//  , double[] D  // D       (input/output) DOUBLE PRECISION array, dimension (N)
//  , double[] E  //E       (input/output) DOUBLE PRECISION array, dimension (N)
//  , double[] E2  //E2      (input/output) DOUBLE PRECISION array, dimension (N)
...
//  , int[] NSPLIT  //NSPLIT  (output) INTEGER
//  , int[] ISPLIT  //ISPLIT  (output) INTEGER array, dimension (N)
//  , int[] M  //M       (output) INTEGER
//  , double[] W  //W       (output) DOUBLE PRECISION array, dimension (N)
//  , double[] WERR  // WERR    (output) DOUBLE PRECISION array, dimension (N)
//  , double[] WGAP  // WGAP    (output) DOUBLE PRECISION array, dimension (N)
//  , int[] IBLOCK  //IBLOCK  (output) INTEGER array, dimension (N)
//  , int[] INDEXW  //INDEXW  (output) INTEGER array, dimension (N)
//  , double[] GERS  //GERS    (output) DOUBLE PRECISION array, dimension (2*N)
//  , double[] PIVMIN  //PIVMIN  (output) DOUBLE PRECISION
//  , double[] WORK  //WORK    (workspace) DOUBLE PRECISION array, dimension (6*N)
//  , int[] IWORK  //IWORK   (workspace) INTEGER array, dimension (5*N)
//  , int[] INFO  //INFO    (output) INTEGER
...
//  INFO[0] = 0; M[0] = 0; NSPLIT[0] = 0; //INTEGER            IL, INFO, IU, M, N, NSPLIT
...
//  //*
//  //*     .. Parameters ..
//  double FAC, FOURTH, FUDGE, HNDRD,MAXGROWTH, PERT; //DOUBLE PRECISION   FAC, FOUR, FOURTH, FUDGE, HALF, HNDRD,MAXGROWTH, ONE, PERT, TWO, ZERO
//  {  HNDRD = 100.0D; PERT = 8.0D; FOURTH = ONE/FOUR; FAC= HALF; MAXGROWTH = 64.0D; FUDGE = 2.0D; } //PARAMETER          ( ZERO = 0.0D0, ONE = 1.0D0,TWO = 2.0D0, FOUR=4.0D0,HNDRD = 100.0D0,PERT = 8.0D0,HALF = ONE/TWO, FOURTH = ONE/FOUR, FAC= HALF,MAXGROWTH = 64.0D0, FUDGE = 2.0D0 )
...
//  int CNT, CNT1, CNT2, I, IBEGIN, IDUM, IEND, IN, INDL, INDU
//    , IRANGE = 0, J, JBLK, MB, MM,WBEGIN, WEND; //INTEGER            CNT, CNT1, CNT2, I, IBEGIN, IDUM, IEND, IINFO,IN, INDL, INDU, IRANGE, J, JBLK, MB, MM,WBEGIN, WEND
//  int[] IINFO = {0};
...
//  //*     ..
//  //*     .. Local Arrays ..
//  int[] ISEED = {0, 0, 0, 0}; //INTEGER            ISEED( 4 )
////*     ..
////*     .. External Functions ..
...
//    if ( (IRANGE == ALLRNG)
//      || ((IRANGE == VALRNG) && (D[1 -1] > VL[0]) && (D[1 -1] <= VU[0]))
//      || ((IRANGE == INDRNG) && (IL == 1) && (IU == 1)) ) { //IF( (IRANGE.EQ.ALLRNG).OR.((IRANGE.EQ.VALRNG).AND.(D(1).GT.VL).AND.(D(1).LE.VU)).OR.((IRANGE.EQ.INDRNG).AND.(IL.EQ.1).AND.(IU.EQ.1)) ) THEN
//      M[0] = 1; //M = 1
//      W[1 -1] = D[1 -1]; //W(1) = D(1)
//      //*           The computation error of the eigenvalue is zero
//      WERR[1 -1] = ZERO; //WERR(1) = ZERO
//      WGAP[1 -1] = ZERO; //WGAP(1) = ZERO
//      IBLOCK[ 1  -1] = 1; //IBLOCK( 1 ) = 1
//      INDEXW[ 1  -1] = 1; //INDEXW( 1 ) = 1
//      GERS[1 -1] = D[ 1  -1]; //GERS(1) = D( 1 )
//      GERS[2 -1] = D[ 1  -1]; //GERS(2) = D( 1 )
//    } // ENDIF
...
//    WERR[I -1] = ZERO; //WERR(I) = ZERO
//    WGAP[I -1] = ZERO; //WGAP(I) = ZERO
//    EABS = ABS( E[I -1] ); //EABS = ABS( E(I) )
//    if ( EABS  >=  EMAX ) { //IF( EABS .GE. EMAX ) THEN
//      EMAX = EABS; //EMAX = EABS
//    } // END IF
//    TMP1 = EABS + EOLD; //TMP1 = EABS + EOLD
//    GERS[ 2*I-1 -1] = D[I -1] - TMP1; //GERS( 2*I-1) = D(I) - TMP1
//    GL =  MIN( GL, GERS[ 2*I - 1 -1]); //GL =  MIN( GL, GERS( 2*I - 1))
//    GERS[ 2*I  -1] = D[I -1] + TMP1; //GERS( 2*I ) = D(I) + TMP1
//    GU = MAX( GU, GERS[2*I -1] ); //GU = MAX( GU, GERS(2*I) )
//    EOLD  = EABS; //EOLD  = EABS
//  } //5    CONTINUE
////*     The minimum pivot allowed in the Sturm sequence for T
//  PIVMIN[0] = SAFMIN * MAX( ONE, EMAX*EMAX ); //PIVMIN = SAFMIN * MAX( ONE, EMAX**2 )
////*     Compute spectral diameter. The Gerschgorin bounds give an
////*     estimate that is wrong by at most a factor of SQRT(2)
//  SPDIAM = GU - GL; //SPDIAM = GU - GL
////*     Compute splitting points
//  DLARRA( N, D, E, E2, SPLTOL, SPDIAM, NSPLIT, ISPLIT, IINFO); //CALL DLARRA( N, D, E, E2, SPLTOL, SPDIAM,NSPLIT, ISPLIT, IINFO )
////*     Can force use of bisection instead of faster DQDS.
////*     Option left in the code for future multisection work.
//  FORCEB = false; //FORCEB = .FALSE.
////*     Initialize USEDQD, DQDS should be used for ALLRNG unless someone
////*     explicitly wants bisection.
//  USEDQD = (( IRANGE == ALLRNG )  &&  (!FORCEB)); //USEDQD = (( IRANGE.EQ.ALLRNG ) .AND. (.NOT.FORCEB))
//  if ( (IRANGE == ALLRNG)  &&  (!FORCEB) ) { //IF( (IRANGE.EQ.ALLRNG) .AND. (.NOT. FORCEB) ) THEN
...
//    DLARRD( RANGE, 'B', N, VL, VU, IL, IU, GERS,BSRTOL, D, E, E2, PIVMIN, NSPLIT, ISPLIT
//      , MM, W, WERR, VL, VU, IBLOCK, INDEXW,WORK, IWORK, IINFO ); //CALL DLARRD( RANGE, 'B', N, VL, VU, IL, IU, GERS,BSRTOL, D, E, E2, PIVMIN, NSPLIT, ISPLIT,MM, W, WERR, VL, VU, IBLOCK, INDEXW,WORK, IWORK, IINFO )
