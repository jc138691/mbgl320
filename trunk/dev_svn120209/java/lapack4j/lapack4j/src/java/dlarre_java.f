...
public static void DLARRE( RANGE, N, VL, VU, IL, IU, D, E, E2,RTOL1, RTOL2, SPLTOL, NSPLIT, ISPLIT, M,W, WERR, WGAP, IBLOCK, INDEXW, GERS, PIVMIN,WORK, IWORK, INFO ) { //SUBROUTINE DLARRE( RANGE, N, VL, VU, IL, IU, D, E, E2,RTOL1, RTOL2, SPLTOL, NSPLIT, ISPLIT, M,W, WERR, WGAP, IBLOCK, INDEXW, GERS, PIVMIN,WORK, IWORK, INFO )
//IMPLICIT NONE
//*
...
SPDIAM = GU - GL; //SPDIAM = GU - GL
if ( ! ((IRANGE == ALLRNG) && ( !FORCEB)) ) { //IF(.NOT. ((IRANGE.EQ.ALLRNG).AND.(.NOT.FORCEB)) ) THEN
...
USEDQD = ( (MB  >  FAC*IN)  &&  ( !FORCEB) ); //USEDQD = ( (MB .GT. FAC*IN) .AND. (.NOT.FORCEB) )
...
if (( (IRANGE == ALLRNG)  &&  ( ! FORCEB) ) || USEDQD) { //IF(( (IRANGE.EQ.ALLRNG) .AND. (.NOT. FORCEB) ).OR.USEDQD) THEN
//*           Case of DQDS
...
if ( USEDQD  &&   !NOREP ) { //IF( USEDQD .AND. .NOT.NOREP ) THEN
