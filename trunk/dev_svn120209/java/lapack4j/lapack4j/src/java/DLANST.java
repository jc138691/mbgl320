...
//    } //PARAMETER          ( ONE = 1.0D+0, ZERO = 0.0D+0 )
  ...
  if (N <= 0) { //IF( N.LE.0 ) THEN
    ...
    for (I = 1; I <= N - 1; I++) { //DO 10 I = 1, N - 1
      ANORM = MAX(ANORM, ABS(D[I -1])); //ANORM = MAX( ANORM, ABS( D( I ) ) )
      ANORM = MAX(ANORM, ABS(E[I -1])); //ANORM = MAX( ANORM, ABS( E( I ) ) )
    } //10    CONTINUE
    ...
    if (N == 1) { //IF( N.EQ.1 ) THEN
      ANORM = ABS(D[1 -1]); //ANORM = ABS( D( 1 ) )
    } else { // ELSE
      ANORM = MAX(ABS(D[1 -1]) + ABS(E[1 -1]), ABS(E[N - 1 -1]) + ABS(D[N -1])); //ANORM = MAX( ABS( D( 1 ) )+ABS( E( 1 ) ),ABS( E( N-1 ) )+ABS( D( N ) ) )
      for (I = 2; I <= N - 1; I++) { //DO 20 I = 2, N - 1
        ANORM = MAX(ANORM, ABS(D[I -1]) + ABS(E[I -1]) + ABS(E[I - 1 -1])); //ANORM = MAX( ANORM, ABS( D( I ) )+ABS( E( I ) )+ABS( E( I-1 ) ) )
      } //20       CONTINUE
    } // END IF
