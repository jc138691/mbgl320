...
//    log.saveToFile(VecToString.toCsv(x) + "\n" + VecToString.toCsv(f), ".", "wf", "P1s_sqrtCR.csv");
  ...
  cZk = new YkLcr(wCR, f, f, 1).calcZk();

  nZk = new YkLcrV2(wCR, f, f, 1).calcZk();
//    FuncVec newZk = new YkLcr(wCR, f, f, 1).calcZk_NEW();
  FuncVec oZk = new YkLcr(wCR, f, f, 1).calcZk_OLD();
...
//    log.saveToFile(VecToString.toCsv(x) + "\n" + VecToString.toCsv(atomZ), ".", "wf", "Z_1_1s.csv");
  ...
  assertEquals(0, Math.abs(DistMaxAbsErr.distSLOW(tZk, cZk)), 6e-9);  // with calcZk_OLD
  log.info("trueZk=", new VecDbgView(tZk));
  log.info("newZk=", new VecDbgView(nZk));
  assertEquals(0, Math.abs(DistMaxAbsErr.distSLOW(tZk, nZk)), 6e-9);  // with calcZk_OLD
