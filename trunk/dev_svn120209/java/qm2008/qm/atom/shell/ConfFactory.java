...
public class ConfFactory {
  ...
  public static LsConf makeLi_1s2_2s_2S(FuncVec f_1s, FuncVec f_2s) {
    // Making Li(1s^2, 2s)
    int L = 0;
    Ls LS = new Ls(L, Spin.SINGLET);
    ShellQ2 sh = new ShellQ2(0, f_1s, L, LS);
    Shell sh2 = new Shell(1, f_2s, L);
    LsConf res = new ShPair(sh, sh2, new Ls(L, Spin.ELECTRON));
    return res;
  }
  public static LsConf makeLi_1s_2s2_2S(FuncVec f_1s, FuncVec f_2s) {
    // Making Li(1s^2, 2s)
    int L = 0;
    Ls LS = new Ls(L, Spin.SINGLET);
