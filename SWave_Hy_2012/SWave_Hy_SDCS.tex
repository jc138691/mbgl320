...
The total scattering system's eigenstates are constructed by the procedure identical to the one used for the target (\ref{Phi_H_Phi_2})
...
Similar to the potential scattering, the solution of the $(\mathcal{N}+1)$-electron Schr\"odinger equation for a given $\Gamma$,
\beq
...
\beq \barr{l}
 ...
 + A \Phi^\Gamma_{\gamma}  \phi_{\gamma}
+ \sum_{\gamma''} K_{\gamma''} A \Phi^\Gamma_{\gamma''}   \overline{\phi}_{\gamma''} ,
\earr \label{Psi_} \eeq
where the channels' trial functions $\{ \phi_\gamma, \overline{\phi}_\gamma \}$ are defined for the open channels by 
\beq
