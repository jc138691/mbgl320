...
public static void DLASSQ( N, X, INCX, SCALE, SUMSQ ) { //SUBROUTINE DLASSQ( N, X, INCX, SCALE, SUMSQ )
//*
...
{  ZERO = 0.0D+0 ; } //PARAMETER          ( ZERO = 0.0D+0 )
//*     ..
//*     .. Local Scalars ..
int IX; //INTEGER            IX
...
for ( 10 IX = 1; 1 + ( N-1 )*INCX; INCX) { //DO 10 IX = 1, 1 + ( N-1 )*INCX, INCX
...
SUMSQ = SUMSQ + ( ABSXI / SCALE )**2; //SUMSQ = SUMSQ + ( ABSXI / SCALE )**2
} // END IF
} // END IF
