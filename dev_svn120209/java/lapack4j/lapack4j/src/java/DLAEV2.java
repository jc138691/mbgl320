...
public static void DLAEV2(
  final double  A      //A       (input) DOUBLE PRECISION
  , final double B     //B       (input) DOUBLE PRECISION
  , final double C     //C       (input) DOUBLE PRECISION
  , final double[] RT1  //RT1     (output) DOUBLE PRECISION
  , final double[] RT2  //RT2     (output) DOUBLE PRECISION
  , final double[] CS1  //CS1     (output) DOUBLE PRECISION
  , final double[] SN1  //SN1     (output) DOUBLE PRECISION
  ...
  //*
  //*     .. Scalar Arguments ..
  CS1[0] = 0; RT1[0] = 0; RT2[0] = 0; SN1[0] = 0; //DOUBLE PRECISION   A, B, C, CS1, RT1, RT2, SN1
  //*     ..
  //*
  //*  Purpose
  //*  =======
...
//    {  HALF = 0.5D ; } //PARAMETER          ( HALF = 0.5D0 )
  ...
  if ( SM < ZERO ) { //IF( SM.LT.ZERO ) THEN
    RT1[0] = HALF*( SM-RT ); //RT1 = HALF*( SM-RT )
    SGN1 = -1; //SGN1 = -1
    //*        Order of execution important.
    //*        To get fully accurate smaller eigenvalue,
    //*        next line needs to be executed in higher precision.
    RT2[0] = ( ACMX / RT1[0] )*ACMN - ( B / RT1[0] )*B; //RT2 = ( ACMX / RT1 )*ACMN - ( B / RT1 )*B
    ...
    RT1[0] = HALF*RT; //RT1 = HALF*RT
    RT2[0] = -HALF*RT; //RT2 = -HALF*RT
    SGN1 = 1; //SGN1 = 1
  } // END IF
  ...
  if ( ACS > AB ) { //IF( ACS.GT.AB ) THEN
    CT = -TB / CS; //CT = -TB / CS
    SN1[0] = ONE / sqrt( ONE+CT*CT ); //SN1 = ONE / SQRT( ONE+CT*CT )
    CS1[0] = CT*SN1[0]; //CS1 = CT*SN1
  } else { // ELSE
    if ( AB == ZERO ) { //IF( AB.EQ.ZERO ) THEN
      ...
      TN = -CS / TB; //TN = -CS / TB
      CS1[0] = ONE / sqrt( ONE+TN*TN ); //CS1 = ONE / SQRT( ONE+TN*TN )
      SN1[0] = TN*CS1[0]; //SN1 = TN*CS1
    } // END IF
  } // END IF
