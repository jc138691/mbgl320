...
public static void DLASQ4( I0, N0, Z, PP, N0IN, DMIN, DMIN1, DMIN2, DN,DN1, DN2, TAU, TTYPE, G ) { //SUBROUTINE DLASQ4( I0, N0, Z, PP, N0IN, DMIN, DMIN1, DMIN2, DN,DN1, DN2, TAU, TTYPE, G )
//*
...
//*
//*     .. Scalar Arguments ..
int I0, N0, N0IN, PP, TTYPE; //INTEGER            I0, N0, N0IN, PP, TTYPE
double DMIN, DMIN1, DMIN2, DN, DN1, DN2, G, TAU; //DOUBLE PRECISION   DMIN, DMIN1, DMIN2, DN, DN1, DN2, G, TAU
//*     ..
//*     .. Array Arguments ..
...
{  CNST1 = 0.5630D0, CNST2 = 1.010D0,CNST3 = 1.050D0 ; } //PARAMETER          ( CNST1 = 0.5630D0, CNST2 = 1.010D0,CNST3 = 1.050D0 )
double QURTR, THIRD, HALF, ZERO, ONE, TWO, HUNDRD; //DOUBLE PRECISION   QURTR, THIRD, HALF, ZERO, ONE, TWO, HUNDRD
{  QURTR = 0.250D0, THIRD = 0.3330D0,HALF = 0.50D0, ZERO = 0.0D0, ONE = 1.0D0,TWO = 2.0D0, HUNDRD = 100.0D0 ; } //PARAMETER          ( QURTR = 0.250D0, THIRD = 0.3330D0,HALF = 0.50D0, ZERO = 0.0D0, ONE = 1.0D0,TWO = 2.0D0, HUNDRD = 100.0D0 )
//*     ..
//*     .. Local Scalars ..
int I4, NN, NP; //INTEGER            I4, NN, NP
double A2, B1, B2, GAM, GAP1, GAP2, S; //DOUBLE PRECISION   A2, B1, B2, GAM, GAP1, GAP2, S
//*     ..
//*     .. Intrinsic Functions ..
...
GAM = DN; //GAM = DN
A2 = ZERO; //A2 = ZERO
if ( Z( NN-5 )  >  Z( NN-7 ) )RETURN; //IF( Z( NN-5 ) .GT. Z( NN-7 ) )RETURN
...
B2 = Z( NP-2 ); //B2 = Z( NP-2 )
GAM = DN1; //GAM = DN1
if ( Z( NP-4 )  >  Z( NP-2 ) )RETURN; //IF( Z( NP-4 ) .GT. Z( NP-2 ) )RETURN
A2 = Z( NP-4 ) / Z( NP-2 ); //A2 = Z( NP-4 ) / Z( NP-2 )
if ( Z( NN-9 )  >  Z( NN-11 ) )RETURN; //IF( Z( NN-9 ) .GT. Z( NN-11 ) )RETURN
...
//*
A2 = A2 + B2; //A2 = A2 + B2
for ( 10 I4 = NP; 4*I0 - 1 + PP; -4) { //DO 10 I4 = NP, 4*I0 - 1 + PP, -4
if ( B2 == ZERO )GO TO 20; //IF( B2.EQ.ZERO )GO TO 20
B1 = B2; //B1 = B2
if ( Z( I4 )  >  Z( I4-2 ) )RETURN; //IF( Z( I4 ) .GT. Z( I4-2 ) )RETURN
B2 = B2*( Z( I4 ) / Z( I4-2 ) ); //B2 = B2*( Z( I4 ) / Z( I4-2 ) )
A2 = A2 + B2; //A2 = A2 + B2
if ( HUNDRD*MAX( B2, B1 ) < A2  ||  CNST1 < A2 ) GO TO 20; //IF( HUNDRD*MAX( B2, B1 ).LT.A2 .OR. CNST1.LT.A2 ) GO TO 20
...
B2 = Z( NP-6 ); //B2 = Z( NP-6 )
GAM = DN2; //GAM = DN2
if ( Z( NP-8 ) > B2  ||  Z( NP-4 ) > B1 )RETURN; //IF( Z( NP-8 ).GT.B2 .OR. Z( NP-4 ).GT.B1 )RETURN
...
A2 = A2 + B2; //A2 = A2 + B2
for ( 30 I4 = NN - 17; 4*I0 - 1 + PP; -4) { //DO 30 I4 = NN - 17, 4*I0 - 1 + PP, -4
if ( B2 == ZERO )GO TO 40; //IF( B2.EQ.ZERO )GO TO 40
B1 = B2; //B1 = B2
if ( Z( I4 )  >  Z( I4-2 ) )RETURN; //IF( Z( I4 ) .GT. Z( I4-2 ) )RETURN
B2 = B2*( Z( I4 ) / Z( I4-2 ) ); //B2 = B2*( Z( I4 ) / Z( I4-2 ) )
A2 = A2 + B2; //A2 = A2 + B2
if ( HUNDRD*MAX( B2, B1 ) < A2  ||  CNST1 < A2 ) GO TO 40; //IF( HUNDRD*MAX( B2, B1 ).LT.A2 .OR. CNST1.LT.A2 ) GO TO 40
...
B2 = B1; //B2 = B1
if ( B2 == ZERO )GO TO 60; //IF( B2.EQ.ZERO )GO TO 60
for ( 50 I4 = 4*N0 - 9 + PP; 4*I0 - 1 + PP; -4) { //DO 50 I4 = 4*N0 - 9 + PP, 4*I0 - 1 + PP, -4
A2 = B1; //A2 = B1
if ( Z( I4 ) > Z( I4-2 ) )RETURN; //IF( Z( I4 ).GT.Z( I4-2 ) )RETURN
B1 = B1*( Z( I4 ) / Z( I4-2 ) ); //B1 = B1*( Z( I4 ) / Z( I4-2 ) )
B2 = B2 + B1; //B2 = B2 + B1
if ( HUNDRD*MAX( B1, A2 ) < B2 ) GO TO 60; //IF( HUNDRD*MAX( B1, A2 ).LT.B2 ) GO TO 60
50       CONTINUE; //50       CONTINUE
60       CONTINUE; //60       CONTINUE
B2 = SQRT( CNST3*B2 ); //B2 = SQRT( CNST3*B2 )
A2 = DMIN1 / ( ONE+B2**2 ); //A2 = DMIN1 / ( ONE+B2**2 )
...
B1 = Z( NN-5 ) / Z( NN-7 ); //B1 = Z( NN-5 ) / Z( NN-7 )
B2 = B1; //B2 = B1
if ( B2 == ZERO )GO TO 80; //IF( B2.EQ.ZERO )GO TO 80
for ( 70 I4 = 4*N0 - 9 + PP; 4*I0 - 1 + PP; -4) { //DO 70 I4 = 4*N0 - 9 + PP, 4*I0 - 1 + PP, -4
if ( Z( I4 ) > Z( I4-2 ) )RETURN; //IF( Z( I4 ).GT.Z( I4-2 ) )RETURN
B1 = B1*( Z( I4 ) / Z( I4-2 ) ); //B1 = B1*( Z( I4 ) / Z( I4-2 ) )
B2 = B2 + B1; //B2 = B2 + B1
if ( HUNDRD*B1 < B2 )GO TO 80; //IF( HUNDRD*B1.LT.B2 )GO TO 80
70       CONTINUE; //70       CONTINUE
80       CONTINUE; //80       CONTINUE
B2 = SQRT( CNST3*B2 ); //B2 = SQRT( CNST3*B2 )
A2 = DMIN2 / ( ONE+B2**2 ); //A2 = DMIN2 / ( ONE+B2**2 )
