...
public class SlaterLr extends Slater {
  ...
  protected double calcKinL(int L, FuncVec wf, FuncVec wf2) {
  ...
  protected double calcKinDrv(Shell sh, Shell SH2 /* make it stand out*/) {
    int L = sh.getWfL();
    if (L != SH2.getWfL())
      return 0.;
    return calcKinDrv(sh.getWf(), SH2.getWf());
  }
  public double calcKinDrv(FuncVec wf, FuncVec wf2) {
    Vec drv = wf.getDrv();
    Vec drv2 = wf2.getDrv();
