...
public static final int FAST_N1_FOR_N23 = 0;
...
public static LsConfs makeSModelDiff(AtomShModelE3 model, FuncArr arr, int minN1ForMaxN2) {
  ...
  for (int n = 0; n < model.getN1(); n++) {
    ...
    if (minN1ForMaxN2 != -1  &&  n > minN1ForMaxN2)  //-1 means all
      continue;
...
public static LsConfs makeSModelClosed(AtomShModelE3 model, FuncArr arr, int minN1ForMaxN2) {
    ...
    for (int n = 0; n < size; n++) {
      ...
      for (int n3 = 0; n3 < size3; n3++) {
        ...
        if (n < n3) {
          ...
          if (minN1ForMaxN2 != -1  &&  n3 > minN1ForMaxN2)  //-1 means all
            continue;
...
public static LsConfs makeSModelSmall(AtomShModelE3 model, FuncArr arr) {
  log.info("-->makeSModel(AtomShModelE3 model=", model);
  LsConfs res = makeSModelDiff(model, arr, FAST_N1_FOR_N23);
  LsConfs closed = makeSModelClosed(model, arr, FAST_N1_FOR_N23);
  res.addAll(closed);
  log.info("res.size()=" + res.size());
  log.info("<--makeSModel(AtomShModelE3 model=", model);
  return res;
